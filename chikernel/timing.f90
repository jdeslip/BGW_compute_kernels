module timing_m
  ...
  real(kind(1.0d0)), private, save :: acctim(2,mtim),tzero(2,mtim)
...
!================================================================================
  ...
  subroutine timget(cpu,wall)
    real(kind(1.0d0)), intent(out) :: cpu,wall
...
!================================================================================
  ...
  subroutine timacc(n,option,tottim,nslices)
    ...
    real(kind(1.0d0)), intent(out), optional :: tottim(2) !> should be present if option=3 or 4
    integer, intent(out), optional :: nslices !> only used if option=3, still optional in that case
    
    real(kind(1.0d0)) :: cpu,wall
